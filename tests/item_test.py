...
def test_item_description_return_string():
    item1 = item(id=1,name="Sword", description="A sword for fighting")
...
def test_item_status_return_boolean():
    item1 = item(id=1,name="Sword", description="A sword for fighting", status=True)
...
def test_item_name_if_empty_gives_error():
    with pytest.raises(ValueError):
        item1 = item(id=1,name="", description="A sword for fighting")
